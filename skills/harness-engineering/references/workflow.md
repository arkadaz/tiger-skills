...
6. **Implement** (Phase 5) — BEFORE: (A) run Codebase Type Discovery — Glob/Grep for ALL existing types (models, configs, drivers, enums), read every matched file, build a Type Inventory; (B) read all `.md` docs (AGENTS.md, PROGRESS.md, DECISIONS.md, GRAPH.md, codebase-map.md, business docs, specs); (C) pass code-quality comprehension gate (read all 13 design principles + language rules from [code-quality](../../code-quality/SKILL.md), pass all 7 self-check questions). INVOKE: `superpowers:subagent-driven-development` or `superpowers:executing-plans`. All code follows `superpowers:test-driven-development` (failing test first, minimal implementation, verify, refactor). All code follows code-quality rules — use real types from the Type Inventory, never `Any` when a type exists. On any failure: invoke `superpowers:systematic-debugging`. DURING: auto-track after every commit.
